 ...
 * 
 * 把 自定义事件data.on 赋值给了 listeners，把浏览器原生事件 data.nativeOn 赋值给了 data.on，
 * 这说明所有的原生浏览器事件处理是在当前父组件环境中处理的。而对于自定义事件，会把 listeners 作为 vnode 的 componentOptions 传入，
 * 放在子组件初始化阶段中处理， 在子组件的初始化的时候， 
